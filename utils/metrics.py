...
# def metric_cal(pixel_scores, image_scores, gt_list, gt_mask_list, cal_pro=False):
#     """
#     (修改版) 分别接收像素级和图像级分数，进行指标计算。
#     """
#     # --- ✨ 关键修改 1: 直接使用传入的 image_scores 进行图像级评估 ---
#     # 删除了旧的 img_scores = scores.reshape(...).max(axis=1)
#     gt_list = np.asarray(gt_list, dtype=int)
#     img_roc_auc = roc_auc_score(gt_list, image_scores)
#     img_f1, img_threshold = calculate_max_f1(gt_list, image_scores)
#
#     # --- 像素级评估逻辑保持不变，但使用 pixel_scores ---
#     gt_mask = np.asarray(gt_mask_list, dtype=int)
#     # F1 score
#     pxl_f1, pxl_threshold = calculate_max_f1(gt_mask.flatten(), pixel_scores.flatten())
#     # ROCAUC
#     per_pixel_rocauc = roc_auc_score(gt_mask.flatten(), pixel_scores.flatten())
#
#     # --- PRO-AUC 计算逻辑保持不变 ---
#     if cal_pro:
#         # 注意：PRO AUC 仍然是基于像素级的异常图计算的
#         pro_auc_score = cal_pro_metric(gt_mask_list, pixel_scores, fpr_thresh=0.3)
#         max_f1_region = calculate_max_f1_region(gt_mask_list, pixel_scores)
#     else:
#         pro_auc_score = 0
#         max_f1_region = 0
#
